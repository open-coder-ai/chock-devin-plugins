...
# DIFFERENT clause of the same command line -- `rm -f /tmp/x; cat .claude/settings.json` is
# a read, not a write, and `echo hi > /tmp/x; cat .claude/settings.json` never touches the
# protected path at all. Word-splitting alone only recognises a separator as ITS OWN token
# ("foo ; bar"), which misses the far more common "foo; bar" style, so a separator glued to
# the END of a token is peeled off too; one glued to a token's start or middle ("foo;bar",
# no space anywhere) is not, the residual limit of scanning words rather than parsing shell.
_clauses() {
    local tok body sep
    local -a cur=()
    set -f
...
# Protected paths, matched against one clause's backslash-normalized text. Includes the
...
# In-place editors (substring) and the writer-token/interpreter-write helpers above, all
# scoped to ONE clause so a writer verb elsewhere in the command cannot pair with a
# protected path it never touches.
_clause_writer() {
    case "$1" in
...
# clause. Without this, `cat .claude/settings.json 2>/dev/null` (stderr suppressed, no
# write anywhere) and `cat .claude/settings.json 2>&1` (fd duplication) were both refused:
# the old check only tested whether SOME '>' and SOME protected substring occurred
...
_redirect_target_protected() {
    ...
    local target_re='(AGENTS\.md|CLAUDE\.md|GEMINI\.md|copilot-instructions\.md|\.cursorrules|\.windsurfrules|\.aider\.conf\.yml|\.claude/settings|\.mcp\.json|\.chock/bin/|\.chock/compiled/|\.git/hooks/|\.agents/policies/[^[:space:]]*implementations)'
